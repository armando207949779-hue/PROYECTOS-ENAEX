...
# =========================================================
# APP
# Estructura solicitada: logo -> subir archivo -> procesamiento -> dashboard
# =========================================================
...
if uploaded_file is None:
    st.info("Carga el archivo con fechas finales para calcular y visualizar Performance TAT.")
    st.stop()
...
try:
    ...
    with f2:
        if "sistema" in df_dashboard.columns:
            sistemas = sorted(df_dashboard["sistema"].dropna().astype(str).unique().tolist())
            sistemas_sel = st.multiselect("Sistema", sistemas, default=sistemas)
        else:
            sistemas_sel = []
    ...
    with f3:
        if col_centro is not None:
            centros = sorted(df_dashboard[col_centro].dropna().astype(str).str.strip().unique().tolist())
            default_centros = ["E002"] if "E002" in centros else centros
            centros_sel = st.multiselect("Centro", centros, default=default_centros)
        else:
            centros_sel = []
            st.info("Sin columna centro")

    with f4:
        perf_options = ["Cumple", "No cumple", "En proceso", "No aplica al análisis", "Sin datos"]
        perf_existentes = [x for x in perf_options if x in df_dashboard["performance_tat_total"].astype(str).unique()] if "performance_tat_total" in df_dashboard.columns else []
        perf_sel = st.multiselect(
            "Performance TAT",
            perf_existentes,
            default=[x for x in ["Cumple", "No cumple"] if x in perf_existentes],
        )
    ...
    if rango_fechas is not None:
        if isinstance(rango_fechas, (tuple, list)) and len(rango_fechas) == 2:
            ...
            fecha_fin = pd.Timestamp(rango_fechas[1]) + pd.Timedelta(days=1) - pd.Timedelta(microseconds=1)
            df_dashboard = df_dashboard[
                df_dashboard[COL_FECHA_RECEPCION_FINAL].notna()
                & df_dashboard[COL_FECHA_RECEPCION_FINAL].between(fecha_inicio, fecha_fin)
            ].copy()

    if "sistema" in df_dashboard.columns and sistemas_sel:
        df_dashboard = df_dashboard[df_dashboard["sistema"].astype(str).isin(sistemas_sel)].copy()

    if col_centro is not None and centros_sel:
        df_dashboard = df_dashboard[
            df_dashboard[col_centro].astype(str).str.strip().isin([str(x).strip() for x in centros_sel])
        ].copy()

    if "performance_tat_total" in df_dashboard.columns and perf_sel:
        df_dashboard = df_dashboard[df_dashboard["performance_tat_total"].astype(str).isin(perf_sel)].copy()
    ...
    with tab_dashboard:
        ...
        cumple_tat = int(df_dashboard["performance_tat_total"].eq("Cumple").sum()) if "performance_tat_total" in df_dashboard.columns else 0
        no_cumple_tat = int(df_dashboard["performance_tat_total"].eq("No cumple").sum()) if "performance_tat_total" in df_dashboard.columns else 0
        evaluables_tat = cumple_tat + no_cumple_tat
        pct_cumple_tat = cumple_tat / evaluables_tat * 100 if evaluables_tat else 0
        incumplimientos_tat = int(df_dashboard["incumplimiento_tat"].eq(True).sum()) if "incumplimiento_tat" in df_dashboard.columns else 0
        ...
        st.caption(
            "Las donas replican la lógica solicitada: se consideran solo estados Cumple / No Cumple de cada etapa, "
            "y el promedio usa únicamente días positivos, Dx > 0."
        )
        ...
        for i, etapa in enumerate(ETAPAS_DASHBOARD):
            ...
            with cols_etapas[i]:
                ...
                st.markdown(f"<div style='font-size:12px;color:#555;min-height:36px'>{etapa['regla']}</div>", unsafe_allow_html=True)
                st.altair_chart(
                ...
                st.markdown(
                    ...
                    <div style="text-align:center; margin-top:-6px;">
                        <div style="font-size:32px; font-weight:800; color:#1f2937;">{datos['promedio']:.0f}</div>
                        <div style="font-size:12px; color:#6b7280;">{etapa['texto_promedio']}</div>
        ...
        with st.expander("Ver tabla resumen por etapa", expanded=False):
            st.dataframe(resumen_etapas_df, use_container_width=True, hide_index=True)
    ...
    with tab_auditoria:
        ...
        with st.expander("Columnas nuevas agregadas", expanded=False):
            st.dataframe(resumen_cols, use_container_width=True, hide_index=True)
        ...
        with st.expander("Top filas con mayor incumplimiento TAT", expanded=False):
            if "dias_incumplimiento_tat" in df_dashboard.columns:
                columnas_top = [
                    "Solicitud de pedido - ME5A", COL_PEDIDO, COL_DOCUMENTO_COMPRAS,
                    "tipo_oc", "origen", "sistema", "dias_tat_total", "umbral_tat_total",
                    "dias_incumplimiento_tat", "rango_incumplimiento_tat", "performance_tat_total",
                    COL_FECHA_SOLICITUD_FINAL, COL_FECHA_RECEPCION_FINAL,
                ]
                columnas_top = [col for col in columnas_top if col in df_dashboard.columns]
                st.dataframe(
                    df_dashboard.sort_values("dias_incumplimiento_tat", ascending=False)[columnas_top].head(int(limite_vista)),
                    use_container_width=True,
                    hide_index=True,
                )
        ...
        with st.expander("Auditoría de días altos por etapa", expanded=False):
            columnas_dias = [
                "dias_liberacion_solped", "dias_comprador", "dias_proveedor", "dias_logistica", "dias_tat_total"
            ]
            columnas_dias = [col for col in columnas_dias if col in df_dashboard.columns]
            if columnas_dias:
                etapa_auditoria = st.selectbox("Selecciona etapa", columnas_dias)
                modo = st.radio("Ordenar por", ["Días más altos", "Días más bajos / negativos"], horizontal=True)
                asc = modo == "Días más bajos / negativos"

                serie = pd.to_numeric(df_dashboard[etapa_auditoria], errors="coerce")
                col_umbral = etapa_auditoria.replace("dias_", "umbral_")
                sobre_umbral = 0
                if col_umbral in df_dashboard.columns:
                    sobre_umbral = int(serie.gt(pd.to_numeric(df_dashboard[col_umbral], errors="coerce")).sum())
                ...
                columnas_auditoria = [
                    "Solicitud de pedido - ME5A", COL_PEDIDO, COL_DOCUMENTO_COMPRAS,
                    "tipo_oc", "origen", "sistema", COL_FECHA_SOLICITUD_FINAL,
                    COL_FECHA_LIBERACION_FINAL, COL_FECHA_PEDIDO_FINAL, COL_FECHA_FACTURACION_FINAL,
                    COL_FECHA_RECEPCION_FINAL, "dias_liberacion_solped", "dias_comprador",
                    "dias_proveedor", "dias_logistica", "dias_tat_total", "performance_tat_total",
                ]
                columnas_auditoria = [col for col in columnas_auditoria if col in df_dashboard.columns]
                st.dataframe(
                    df_dashboard.assign(_orden=serie).sort_values("_orden", ascending=asc).drop(columns=["_orden"])[columnas_auditoria].head(int(limite_vista)),
                    use_container_width=True,
                    hide_index=True,
                )
            else:
    ...
    with tab_datos:
        ...
        st.caption(f"Mostrando hasta {int(limite_vista):,} registros de {len(df_original):,} originales.")
        ...
        columnas_preferidas = [
            "Solicitud de pedido - ME5A", COL_PEDIDO, COL_DOCUMENTO_COMPRAS,
            "tipo_oc", "origen", "sistema", "nombre_tipo_compra", "monto",
            COL_FECHA_SOLICITUD_FINAL, COL_FECHA_LIBERACION_FINAL, COL_FECHA_PEDIDO_FINAL,
            COL_FECHA_FACTURACION_FINAL, COL_FECHA_RECEPCION_FINAL,
            "dias_liberacion_solped", "dias_comprador", "dias_liberacion_pedido",
            "dias_proveedor", "dias_logistica", "dias_tat_total",
            "umbral_liberacion_solped", "umbral_comprador", "umbral_proveedor",
            "umbral_logistica", "umbral_tat_total",
            "performance_liberacion_solped", "performance_comprador", "performance_proveedor",
            "performance_logistica", "performance_tat_total", "tiene_fechas_inconsistentes",
            "dias_incumplimiento_tat", "incumplimiento_tat", "rango_incumplimiento_tat",
        ]
        columnas_preferidas = [col for col in columnas_preferidas if col in df_dashboard.columns]
        st.dataframe(df_dashboard[columnas_preferidas].head(int(limite_vista)), use_container_width=True, hide_index=True)
    ...
    with tab_descarga:
        ...
        with col_excel:
            ...
            if len(df_final) > limite_excel:
                st.warning(f"Excel no disponible porque la salida supera {limite_excel:,} filas. Usa Parquet o CSV.")
            else:
