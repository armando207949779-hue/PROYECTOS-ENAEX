...
def preparar_consolidado_resumido(df_consolidado):
    ...
    df = df.rename(columns={
        "MOP indice precios consumidor": "MOP 1 (Índice de precios al consumidor)",
        "MOP indice remuneraciones": "MOP 2 (Índice de remuneraciones)",
        "MOP petroleo diesel": "MOP 3 (Petróleo Diesel)",
        "MOP dolar observado": "MOP 22 (Dólar observado)",
        "MOP petroleo diesel refineria concon": "MOP 27 (Petróleo Diesel refinería CONCÓN)"
    })
...
def calcular_item_2_item_3(df_resumido):
    ...
    columna_mop_27 = "MOP 27 (Petróleo Diesel refinería CONCÓN)"
    ...
    df[columna_mop_27] = pd.to_numeric(
        df[columna_mop_27],
        errors="coerce"
    )
